...
\subsection{倒映空间}
    ...
    坐标空间的原胞的平移矢量写为$\vec{R}=l_1\vec{a_1}+l_2\vec{a_2}+l_3\vec{a_3}$，其中$\vec{a_i}$为原胞对应方向的晶格基矢；类似的，我们定义晶格中的任意波矢$\vec{k}=k_1\vec{b_1}+k_2\vec{b_2}+k_3\vec{b_3}$，其中$\vec{b_j}$为波矢空间中的基矢，称为\textbf{倒格子基矢}\index{倒格子基矢}，且$a_i$、$b_j$满足：
...
\subsection{倒格子的性质}
    对于正格子及其平移矢量$\vec{R_l}=l_1\vec{a_1}+l_2\vec{a_2}+l_3\vec{a_3}$，所对应的倒格子及任意波矢$\vec{k}=k_1\vec{b_1}+k_2\vec{b_2}+k_3\vec{b_3}$，归纳总结以下性质：
